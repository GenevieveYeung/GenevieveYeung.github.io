...
const filters = ["ALL", "MACHINE LEARNING", "COMPUTER VISION", "LLM APPLICATIONS", "DATA ANALYTICS", "AI SYSTEMS", "BIOMEDICAL ENGINEERING"] as const;
type ProjectFilter = (typeof filters)[number];
const filterTags: Record<Exclude<ProjectFilter, "ALL">, string> = { "MACHINE LEARNING": "Machine Learning", "COMPUTER VISION": "Computer Vision", "LLM APPLICATIONS": "LLM Applications", "DATA ANALYTICS": "Data Analytics", "AI SYSTEMS": "AI Systems", "BIOMEDICAL ENGINEERING": "Biomedical Engineering" };
const projectTaxonomy: Record<string, string[]> = {
  "vgrf-koa-prediction": ["Machine Learning", "Computer Vision", "Biomedical Engineering"],
  medisim: ["LLM Applications", "AI Systems"],
  "fall-detection-system": ["Computer Vision", "Machine Learning"],
  emopet: ["Machine Learning", "Biomedical Engineering", "LLM Applications"],
  parkincare: ["AI Systems", "Machine Learning", "Computer Vision", "Biomedical Engineering"],
  "colon-gland-segmentation": ["Computer Vision", "Biomedical Engineering"],
  "aiot-edge-cloud-scheduling": ["AI Systems", "Machine Learning"],
};
...
function ProjectDetails({ project, cover, locale }: { project: (typeof projectLibrary)[number]; cover: string | null; locale: "en" | "zh-CN" | "zh-HK" }) {
  ...
  return <div className="project-dossier project-detail-body project-row-body">
    ...
    <div className="project-dossier-support"><div><span className="project-detail-label">{locale === "en" ? "My Contribution" : "我的工作"}</span><p>{localized.contribution.split("\n\n").map((paragraph, index) => <span className="project-copy-paragraph" key={`${project.slug}-contribution-${index}`}>{paragraph}</span>)}</p></div><div><span className="project-detail-label">{locale === "en" ? "Methods & Tools" : locale === "zh-CN" ? "方法与工具" : "方法與工具"}</span><p>{localized.methods?.map(method => <span className="project-method" key={method}>{method}</span>) ?? project.tools}</p></div></div>
...
export default function ProjectExperienceAccordion({ media }: { media: Record<string, string | null> }) {
  ...
  const visibleProjects = activeFilter === "ALL" ? projectLibrary : projectLibrary.filter(project => projectTaxonomy[project.slug]?.includes(filterTags[activeFilter]));
  const filterLabel = (filter: ProjectFilter) => filter === "ALL" ? copy.projects.filters.all : filter === "MACHINE LEARNING" ? copy.projects.filters.machineLearning : filter === "COMPUTER VISION" ? copy.projects.filters.computerVision : filter === "LLM APPLICATIONS" ? copy.projects.filters.llmApplications : filter === "DATA ANALYTICS" ? copy.projects.filters.dataAnalytics : filter === "AI SYSTEMS" ? copy.projects.filters.aiSystems : copy.projects.filters.biomedicalEngineering;
  return <div className="project-experience-browser">
    ...
    <LayoutGroup id="experience-focus"><div className="project-list" aria-live="polite">{visibleProjects.map(project => { const isOpen = activeProjectId === project.slug; const cover = media[project.slug] || null; const isQuiet = activeProjectId !== null && !isOpen; const localized = projectContent[locale][project.slug] ?? projectContent.en[project.slug]; const taxonomy = projectTaxonomy[project.slug] ?? project.tags; return <ExperienceIndexShell key={project.slug} id={project.slug} kind="project" number={project.number} icon={<ProjectIcon slug={project.slug} />} period={taxonomy[0] || "Project study"} title={localized.title} supporting={`${project.metric} · ${localized.detail}`} open={isOpen} quiet={isQuiet} detailId={`project-${project.slug}`} onToggle={() => setActiveProjectId(current => current === project.slug ? null : project.slug)}><ProjectDetails project={project} cover={cover} locale={locale} /></ExperienceIndexShell>; })}</div></LayoutGroup>
  </div>;
}
