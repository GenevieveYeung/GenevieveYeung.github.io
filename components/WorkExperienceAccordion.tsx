...
import { workContent } from "@/data/i18n";
...
export default function WorkExperienceAccordion() {
  ...
  const workItems: WorkItem[] = workContent[locale];
  return <LayoutGroup id="experience-focus"><div className="work-accordion"><WorkChronologyGutter /><div className="work-chronology-list">{workItems.map((item, index) => { const key = `${item.org}-${index}`; const isOpen = activeExperienceId === key; const isQuiet = activeExperienceId !== null && !isOpen; return <ExperienceIndexShell key={key} id={key} kind="work" number={String(index + 1).padStart(2, "0")} icon={<WorkIcon index={index} />} period={item.period} title={item.role} supporting={item.org} open={isOpen} quiet={isQuiet} detailId={`work-detail-${index}`} onToggle={() => setActiveExperienceId(isOpen ? null : key)}><div className="work-dossier"><aside className="work-detail-rail"><span>{item.tag}</span><i aria-hidden="true" /></aside><div className="work-details__content"><ul className="work-responsibilities">{item.bullets.map(bullet => <li key={bullet}>{bullet}</li>)}</ul></div></div></ExperienceIndexShell>; })}</div></div></LayoutGroup>;
}
